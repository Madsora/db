...
from messages.messagelistener import EventListener
    ...
    def run(self):
        while True:
            ...
            if message:
                ...
                message = self.conn.hmget("message:%s" % message_id, [
                ...
                message_text = self.conn.hmget(
                    "message:%s" % message_id, ["text"])[0]
                is_spam = "spam" in message_text
                pipeline = self.conn.pipeline(True)
                pipeline.hincrby("user:%s" % sender_id, "checking", -1)
                if is_spam:
                    print("The message with id %d was blocked. It is spam" %
                          message_id)
                    sender_username = self.conn.hmget(
                    ...
                    print("The message with id %d was checked and sent." %
                          message_id)
                    pipeline.hmset('message:%s' % message_id, {
                        'status': 'sent'
                    })
