...
logging.basicConfig(filename="messangers.log", level=logging.INFO)
...
def user_menu_flow(connection, current_user_id):
    while True:
        ...
        if choice == 1:
            ...
            connection.publish('users', "User %s signed out" % connection.hmget(
                "user:%s" % current_user_id, ["login"])[0])
            break
            ...
            try:
                ...
                print("Can't find user with this username")
...
def register(conn, username):
    ...
    pipeline.execute()
    logging.info(
        f"{datetime.datetime.now()} Actor: {username} Action: register \n")
    return user_id
...
def sign_in(conn, username) -> int:
    ...
    if not user_id:
        print("User with this username doesn't exist. Please, register first")
        return -1

    conn.sadd("online:", username)
    logging.info(
        f"{datetime.datetime.now()} Actor: {username} Action: log in \n")
...
def sign_out(conn, user_id) -> int:
    ...
    logging.info(
        f"{datetime.datetime.now()} Actor: {username} Action: sign out \n")
